...
export default function Layout({ children, currentPageName }) {
  ...
  const filteredNavItems = navigationItems.filter((item) => {
    // If the item is not admin-only, always show it
    if (!item.adminOnly) return true;

    // If there's no user loaded yet, hide admin-only items
    if (!user) return false;

    // Show admin-only items to admins
    if (user.role === 'admin') return true;

    // Also allow users with an explicit permission flag to see admin items
    // (e.g., user.permissions.manage_clients)
    if (user.permissions && user.permissions.manage_clients) return true;
