...
export default function ChatWidget() {
  ...
  useEffect(() => {
    ...
    // Send resize message to parent window for responsive design
    const sendResizeMessage = () => {
      if (window.parent !== window) {
        window.parent.postMessage({
          type: 'supportai-resize',
          width: '100vw',
          height: '100vh',
          isOpen: isOpen
        }, '*');
    ...
    // Send size updates when widget state changes
  ...
  const { data: messages } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        const response = await fetch(`${API_CONFIG.WIDGET_URL}/messages/${currentConversation.id}`);
  ...
  const createConversationMutation = useMutation({
    mutationFn: async (data) => {
      try {
        const response = await fetch(`${API_CONFIG.WIDGET_URL}/conversation`, {
          method: 'POST',
          headers: {
...
Provide a helpful response:`,
      ...
      await fetch('${API_CONFIG.WIDGET_URL}/message', {
        ...
        body: JSON.stringify({
          conversation_id: currentConversation._id || currentConversation.id,
          sender_type: 'bot',
          sender_name: 'AI Assistant',
      ...
      if (aiResponse.should_handoff || aiResponse.confidence < (clientSettings?.confidence_threshold || 0.7)) {
        await fetch(`${API_CONFIG.WIDGET_URL}/conversation/${currentConversation.id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const handleRequestAgent = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_CONFIG.WIDGET_URL}/conversation/${currentConversation.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
