...
export default function Integration() {
  ...
  const widgetUrl = selectedClient 
    ...
    : `${window.location.origin}/ChatWidget?client_id=demo-client`;

  const handlePreviewWidget = () => {
    const url = widgetUrl;
    console.log('Opening widget URL:', url);
    if (url) {
      window.open(url, '_blank', 'width=400,height=600,scrollbars=yes,resizable=yes');
    } else {
      alert('Unable to generate preview URL. Please check your client configuration.');
    }
  };

  const handlePreviewDemo = () => {
    const demoUrl = `${window.location.origin}/widget-demo.html`;
...
<script>
  (function() {
    ...
    // Message handling for responsive design
    window.addEventListener('message', function(event) {
      if (event.origin !== '${window.location.origin}') return;
      if (event.data.type === 'supportai-resize') {
        iframe.style.width = event.data.width || '100vw';
        iframe.style.height = event.data.height || '100vh';
      }
    });
  })();
</script>`;
    ...
    <div className="p-6 lg:p-8 max-w-6xl mx-auto space-y-6">
      ...
      <Tabs defaultValue="embed" className="space-y-6">
        ...
        <TabsContent value="script">
          <Card>
            <CardHeader>
              <CardTitle>Simple Script Integration (Recommended)</CardTitle>
            </CardHeader>
            <CardContent className="space-y-6">
              <div>
                ...
                <p className="text-sm text-slate-600 mb-3">
                  The easiest way to add the widget - just one script tag!
                </p>
                <div className="relative">
                  <pre className="bg-slate-900 text-slate-100 p-4 rounded-lg overflow-x-auto text-sm">
                    <code>{`<script src="${window.location.origin}/supportai-widget.js" data-client-id="${selectedClient?.id || 'demo-client'}"></script>`}</code>
                  </pre>
                  <Button
                    size="sm"
                    className="absolute top-2 right-2"
                    onClick={() => handleCopy(`<script src="${window.location.origin}/supportai-widget.js" data-client-id="${selectedClient?.id || 'demo-client'}"></script>`)}
                  >
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <h4 className="font-semibold text-green-800 mb-2">✅ Benefits of Script Integration</h4>
                <ul className="text-sm text-green-700 space-y-1">
                  <li>• Automatic updates when you improve the widget</li>
                  <li>• Better performance and caching</li>
                  <li>• Easier to manage across multiple pages</li>
            ...
            <CardContent className="space-y-6">
              ...
              <div>
                ...
                <div className="mb-3 p-2 bg-gray-50 rounded text-xs">
                  <strong>Widget URL:</strong> {widgetUrl}
                </div>
                
                <div className="flex gap-3">
                  <Button variant="outline" onClick={handlePreviewWidget}>
                    <ExternalLink className="w-4 h-4 mr-2" />
                    Preview Widget
                  </Button>
                  <Button variant="outline" onClick={handlePreviewDemo}>
                    <Globe className="w-4 h-4 mr-2" />
                    Full Demo Page
                  </Button>
                </div>
